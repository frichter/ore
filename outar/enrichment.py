...
class Enrich(object):
    ...
    def __init__(self, var_loc, expr_outs_loc, enrich_loc, rv_outlier_loc,
        ...
        Args:
            ...
            `annovar_func`: annovar function to filter on (default None)

        Attributes:
            `joined_df`: variants and outliers in a single dataframe

        TODO:
            un-hard code `rv_outlier_loc`
    ...
    def load_vars(self):
        ...
        TODO:
            Alternatively parallelize this step as well
            possibly low_memory=False for var_df_per_chrom = pd.read_table
            figure out how to pass regex to str.contains (so exonic does not
                also call e.g., ncRNA_exonic)
        ...
        self.var_df['var_id_freq'] = self.var_df.groupby(
    ...
    def load_outliers(self):
        ...
        "wgs_pcgc_singletons_outliers.txt"
    ...
    def loop_enrichment(self):
        """Loop over enrichment.

        TODO:
            Find pandas/python equivalent of expand.grid, a cartesian product
            https://stackoverflow.com/questions/533905/get-the-cartesian-product-of-a-series-of-lists
            Save output dataframe
            `max_within_pop_freq` not hard-coded (eg function of n with
                some minimum specified)
        ...
        max_within_pop_freq = 5
        cartesian_iter = itertools.product(expr_cut_off_vec, tss_cut_off_vec,
                                           af_cut_off_vec)
        enrichment_per_tuple_partial = partial(
            self.enrichment_per_tuple, max_within_pop_freq=max_within_pop_freq)
        # run either multi-core or single core
        if run_multi_core:
            print("Using 12 cores, less than all {} cores".format(cpu_count()))
            with Pool(12) as p:
    ...
    def enrichment_per_tuple(self, cut_off_tuple, max_within_pop_freq):
    ...
    @staticmethod
    def identify_rows_to_keep(joined_df, max_within_pop_freq, distribution,
                              cut_off_tuple):
        ...
        TODO:
            Can we assume that each gene has at least 1 rare variant?
            Keep variants with pop_max == 1.0?
        ...
        joined_df["rare_variant_status"] = (
            ...
            joined_df.var_id_freq <= max_within_pop_freq)
        # Only keep genes with at least 1 rare variant.
        # joined_df = joined_df[
    ...
    def write_rvs_w_outs_to_file(self, out_cut_off, tss_cut_off, af_cut_off):
        ...
        outlier_df = self.identify_rows_to_keep(
            enrich_df, max_within_pop_freq=5,
